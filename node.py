import socket, threading, time, math, os, random
...
class Node():
    ...
    def __generate_ID(self) -> int:
        ...
        if self.__ID == self.__ID_MAX - 1:
            ...
            if self.__comm_initated:
                ...
                self.force_finish_comm()

            self.__ID = self.__restart_ID()
        else:
            self.__ID += 1
    ...
    def set_input_size(self, size: int) -> None:
        ...
        request = Frame(ID=self.__generate_ID(), target=size, SET=True)
        self.__send_and_request(frame=request)
    ...
    def get_file_corruption(self) -> float:
        return self.__file_corruption
    
    def set_file_corruption(self, corruption: float) -> None:
        ExceptionHandler.check_comp(obj=corruption, name="corruption", type=float, min=0, max=1)

        self.__file_corruption = corruption

    def get_frame_corruption(self) -> float:
        return self.__frame_corruption
    
    def set_frame_corruption(self, corruption: float) -> None:
        ExceptionHandler.check_comp(obj=corruption, name="corruption", type=float, min=0, max=1)

        self.__frame_corruption = corruption
    ...
    def swap_roles(self):
        ...
        swapper = Frame(ID=self.__generate_ID(), SET=True)
    ...
    def start_comm(self, ip: str, port: int, requested: bool = False, interval=1.5) -> bool:
        ...
        if requested:
            initializer = Frame(ID=self.__generate_ID(), target=self.__max_input_size, SYN=True)
        ...
        while not self.__listen_stop_signal.is_set():
            ...
            try:
                ...
                if frame.get_crc_dec() != frame.calculate_FCS():
                    ...
                    frame.frame()
                    # NACK_response = Frame(ID=self.__generate_ID(), target=frame.get_ID_dec(), NACK=True)
                    ...
                    response = Frame(ID=self.__generate_ID(), target=frame.get_ID_dec(), ACK=True)
                    ...
                    if not self.__comm_requested:
                        response = Frame(ID=self.__generate_ID(), target=self.get_input_size(), SYN=True)
                        
                        thread = threading.Thread(target=self.start_comm, args=(self.__node_2[0], self.__node_2[1]))
                        thread.start()

                        self.__send_and_request(response)
                    ...
                    elif responded_f.is_set() and responded_f.get_target_dec() == 0 and not responded_f.contains_payload():
                        self.__swap_roles()
                        print("Role swapped.")
                        self.print_role()
                    elif responded_f.is_set() and responded_f.get_target_dec() > 0 and not responded_f.contains_payload():
                    ...
                    if not self.__comm_requested:
                        ...
                        response = Frame(ID=self.__generate_ID(), FIN=True)
                        self.__send_and_request(response)
                    ...
                    #print(f"Result of pushing a node to a sequence: {result}")
                            ...
                            with open(file_name, 'wb') as file:
                                ...
                                while cur:
                                    ...
                                    if frag_count == 0 and payload_len == 0:
                                        file.write(cur.frame.get_payload()[len(file_name) + 1 :])
                                    else:
                ...
                #Handling NACK messages here
                    ...
                    response: Frame = Frame(ID=self.__generate_ID(), target=frame.get_ID_dec(), ACK=True)
                    ...
                    with open(self.__extract_file_name(payload=frame.get_payload()), 'w') as file:                            
                        file.write(frame.get_payload_str()[len(file_name) + 1 :])
    ...
    def __send_control_messages(self, sleep_interval: int = 10):
        header = Frame(ID=self.__generate_ID(), CONTROL=True)
        ...
        while not self.__keep_alive_stop_signal.is_set():
            # Break the sleep interval into smaller chunks (e.g., 1 second)
            for _ in range(sleep_interval):
                time.sleep(1)
                if self.__keep_alive_stop_signal.is_set():
                    break  # Exit the loop if the stop signal is set

            if self.__keep_alive_stop_signal.is_set():
                break  # Exit the outer loop if the stop signal is set
            ...
            if ARQ_top == -1:
                ...
                header = Frame(ID=self.__generate_ID(), target=self.__KEEP_ALIVE_ARQ.top(), CONTROL=True)
    ...
    def finish_comm(self, requested: bool=False):
        ...
        if requested:
            self.__comm_requested = True
            terminator = Frame(ID=self.__generate_ID(), FIN=True)
            self.__send_and_request(frame=terminator)
    ...
    def force_finish_comm(self):
        ...
        terminator = Frame(ID=1, QUIT=True)
    ...
    def __send_and_request(self, frame: Frame, corrupt: bool = False, _sleep: float = 0.1):
        ...
        if corrupt:
            with self.__lock:
                self.__sock.sendto(Frame.corrupt_data(frame=bytearray(frame.byte_frame)), self.__node_2)
        else:
            with self.__lock:
                self.__sock.sendto(frame.byte_frame, self.__node_2)


    # def corrupt_send_wait(self, frame: Frame):
    #     while not self.__unresponded_ARQ.push(frame):
    #         print("The buffer with unresponded frames is full. Waiting...")
    #         time.sleep(0.1)
        
    #     corrupted_f = Frame.corrupt_data(frame=bytearray(frame.byte_frame))

    #     with self.__lock:
    #         self.__sock.sendto(corrupted_f, self.__node_2)
    ...
    def send_message(self, message : str):
        ...
        if payload_size <= self.__max_output_size:
            ...
            frame = Frame(self.__generate_ID(), target=0, payload=message)
            self.__send_and_request(frame = frame)
            ...
            for frag_i in range(0, frag_count):
                ...
                frame = Frame(self.__generate_ID(), target=prev_frame_ID, MORE_FRAGS=True, payload=message[cur:end])
                ...
                if frag_i == 0:
                    frame.frame()
                    self.__send_and_request(frame=frame, corrupt=True)
                else:
                    self.__send_and_request(frame=frame)
    ...
    def send_file(self, f_path: str, name: str = None):
            ...
            with open(f_path, 'rb') as file:
                ...
                while data:
                    ...
                    if data_count == 0 and frag_count == 0:
                        data_frame = Frame(ID=self.__generate_ID(), payload=bytearray(file_name + data), SET=True, MORE_FRAGS=True)
                    else:
                        data_frame = Frame(ID=self.__generate_ID(), target=prev, payload=bytearray(data), SET=True, MORE_FRAGS=True)
        ...
        if self.__frame_corruption > 0:
            corrupted_frags = random.sample(range(0, frag_count), math.floor(frag_count * self.__frame_corruption ))
    ...
    def __init__(self, port: int = 5000, ip_addr : str = "127.0.0.1", port_2 : int = None, ip_addr_2: str = "127.0.0.1",
                 max_input_size: int = 1000, input_file_path: str = './input_file.txt', output_file_path: str = './output_file.txt', corruption: float = 0.10,
                 file_corruption: float = 0.10):
                 #is_sender : bool = True):
        ...
        self.__frame_corruption: float = corruption
        self.__file_corruption: float = file_corruption
...
#                     case "--corruptness":
#                         if len(args) == 1:
#                             print(node.get_corruptness())
#                         elif len(args) == 2:
#                             node.set_corruptness(float(args[1]))
...
#     --ip <VALUE>:       If value parameter is not present, the current will be printed, otherwise reset.
#     --port <VALUE>:     If value parameter is not present, the current will be printed, otherwise reset.
#     --start_server:     Starts the server and all subprocesses.
#     --quit_server:      Quits the server and all subprocesses.
#     -c <IP> <PORT>:     Starts a communication with a node with the specified parameters.                         
#     -tc:                Closes the started communication with the other node.
#     -ftc:               Forces the communication to close, can only be executed during serious scenarios.
#     -m <VALUE>:         Sends a message to the other node.
#     -f <PATH> <NAME>:   Sends a file with the specified path to the other site. User can also provide
